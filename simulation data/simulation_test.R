...
index <- expand.grid(method_index = method_index, npc_index = npc_index, ncell_index = ncell_index,
...
network_fun <- function(dta_list, method_index, npc_index, ncell_index, tensor_index, nq_index, community_index){
  ...
  if (community_index == "AP"){
    ...
    res <- MASE_fun(net_list, di = 5, K = 4, d_method = "SCORE", c_method = "kmeans")
  } else if (community_index == "M_SLIM"){
    res <- MASE_fun(net_list, di = 5, K = 4, d_method = "SLIM", c_method = "kmeans")
  } else if (community_index == "M_SCORE_AP"){
    res <- MASE_fun(net_list, di = 5, K = 4, d_method = "SCORE", c_method = "AP")
  } else if (community_index == "M_SCORE_LPA"){
    res <- MASE_fun(net_list, di = 5, K = 4, d_method = "SCORE", c_method = "LPA")
  } else if (community_index == "M_SLIM_AP") {
    res <- MASE_fun(net_list, di = 5, K = 4, d_method = "SLIM", c_method = "AP")
  } else if (community_index == "M_SLIM_LPA"){
    res <- MASE_fun(net_list, di = 5, K = 4, d_method = "SLIM", c_method = "LPA")
  }
  # return results
  return(rand.index(res, true_cluster))
}
...
cl <- makeCluster(4) # 4 workers
registerDoParallel(cl)

set.seed(0104)
out_res <- foreach(iter = 1:nrow(index)) %dorng% {
  network_fun(dta_list, method_index = index[iter, 1], npc_index = index[iter, 2], ncell_index = index[iter, 3], 
              tensor_index = index[iter, 4], nq_index = index[iter, 5], community_index = index[iter, 6])
}
stopCluster(cl)
